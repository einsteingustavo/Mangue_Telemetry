...
class Receiver(threading.Thread):
    ...
    def checkData(self):
        ...
        if pckt[0] == 22:
            ...
            print(f"latitude={latitude[-1]}")
            longitude.append(pckt[15])
            print(f"latitude={longitude[-1]}")
        ...
        msg = 14*[0.]
        msg[0] = int(car_save[-1])
        msg[1] = accx_save[-1]
        msg[2] = accy_save[-1]
        msg[3] = accz_save[-1]
        msg[4] = rpm_save[-1]
        msg[5] = speed_save[-1]
        msg[6] = temp_motor_save[-1]
        msg[7] = flags_save[-1]
        msg[8] = soc_save[-1]
        msg[9] = temp_cvt_save[-1]
        msg[10] = volt_save[-1]
        msg[11] = latitude_save[-1]
        msg[12] = longitude_save[-1]
        msg[13] = timestamp_save[-1]
        #publish(self.client, topic, msg)
        #try:
            #self.conn.execute("INSERT INTO aquisitions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                              #(self.id_count, msg[0], msg[1], msg[2], msg[3], msg[4], msg[5], msg[6], msg[7],
...
class Ui_MainWindow(object):
        ...
        while True:
            ...
            if stop_threads:
                try:
                    ...
                    print("Não existe porta COM conectada!")
                break
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.update_map((-12.69638889, -38.32333333))
